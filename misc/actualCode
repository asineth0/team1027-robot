...
public class Robot extends TimedRobot {
  ...
  protected static final int MOTOR_DRIVE_FR_ID = 2; // on god on god
  protected static final int MOTOR_DRIVE_BL_ID = 1;
  protected static final int MOTOR_DRIVE_BR_ID = 3;
  protected static final int MOTOR_TURRET_SHOOT_1_ID = 7;
  protected static final int MOTOR_TURRET_SHOOT_2_ID = 8;
  protected static final int MOTOR_CLIMB_1_ID = 10;
  protected static final int MOTOR_CLIMB_2_ID = 11;
  protected static final int MOTOR_TURRET_ROTATE_ID = 4;
  protected static final int MOTOR_TURRET_AIM_1_ID = 5;
  protected static final int MOTOR_TURRET_AIM_2_ID = 6;
  protected static final int MOTOR_CARGO_ID = 9;
  protected static final int CTL_XBOX_ID = 2;
  ...
  protected static final int CTL_STICK_ID = 0;
  ...
  protected static final int SHOOT_AIM_RESET_LEFT_ID = 0;
  protected static final int SHOOT_AIM_RESET_RIGHT_ID = 1;
  protected static final int HUB_LIMIT_LEFT_ID = 2;
  protected static final int HUB_LIMIT_RIGHT_ID = 3;
  protected static final int CARGO_CHECK_ID = 4;
  ...
  public void robotInit() {
    ...
    motor_cargo = new VictorSPX(MOTOR_CARGO_ID);
    ...
    shoot_aim_reset_left = new DigitalInput(SHOOT_AIM_RESET_LEFT_ID);
    shoot_aim_reset_right = new DigitalInput(SHOOT_AIM_RESET_RIGHT_ID);
    hub_limit_left = new DigitalInput(HUB_LIMIT_LEFT_ID);
    hub_limit_right = new DigitalInput(HUB_LIMIT_RIGHT_ID);
    cargo_check = new DigitalInput(CARGO_CHECK_ID);
  ...
  public void teleopPeriodic() {
    ...
    // climb
    boolean pis1Active = false;
    boolean pis2Active = false;

    if(mode_climb == 1) {
      if(ctl_stick.getRawButton(CTL_STICK_BUTTON_8)) {
        pis1Active = !pis1Active;
      }
      if(ctl_stick.getRawButton(CTL_STICK_BUTTON_9)) {
        pis2Active = !pis2Active;
      }
    ...
    if(mode_climb == 0) {
      float aimAngle = (float)-(ctl_stick.getRawAxis(CTL_STICK_AXIS_KNOB) * 90);
      float speed = (float)(aimAngle - pot_shoot_aim_deg) / 90;
      // if (aimAngle - pot_shoot_aim_deg >= 5) {
      //   motor_turret_aim_1.set(ControlMode.PercentOutput, speed);
      //   motor_turret_aim_2.set(ControlMode.PercentOutput, speed);
      // } else if (aimAngle - pot_shoot_aim_deg <= -5) {
      //   motor_turret_aim_1.set(ControlMode.PercentOutput, -speed);
      //   motor_turret_aim_2.set(ControlMode.PercentOutput, -speed);
      // } else {
      ...
      } else if(hub_limit_right.get() && ctl_stick.getRawAxis(CTL_STICK_AXIS_X)>0) {
        motor_turret_rotate.set(ControlMode.PercentOutput, 0);
      } else {
    ...
    SmartDashboard.putNumber("Shooter Angle", pot_shoot_aim_deg);
    SmartDashboard.putBoolean("Piston 1 Active", pis1Active);
    SmartDashboard.putBoolean("Piston 2 Active", pis2Active);
